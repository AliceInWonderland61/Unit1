#Problem1: Given two lists of strings artist and set_times of length n, 
...
"""
def get_artist_info(artist, festival_schedule):
    #is artist inside festival_schedule 
...
"""
def total_sales(ticket_sales):
    #we would create a variable total count and set it to 0
    #we would parse through the values
    #add the values of each item in the dictionary
    #return the total number of tickets sold

    total = 0

    for num in ticket_sales.values():
        total += num
    return total 
...
accepts two dictionaries venue1_schedule and venue2_schedule each mapping the artists playing at the venue to 
their set times. Return a dictionary containing the key-value pairs that are the same in each schedule.
...
"""
def identify_conflicts(venue1_schedule, venue2_schedule):
    #create a dictionary that will save all the conflicted artist schedules
    dict = {}
    #traverse through both venues and compare their keys and their value 
    
    for i in venue1_schedule.keys():
        #check if the current key-meaning artist- is inside the second venue 
        if i in venue2_schedule.keys() and venue1_schedule[i]==venue2_schedule[i]:
            dict[i]=venue1_schedule[i]
    return dict
